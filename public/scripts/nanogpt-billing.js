...
/**
 * Formats a NanoGPT cache TTL value.
 * @param {unknown} value Raw TTL value
 * @returns {string|null}
 */
function formatTtl(value) {
    const numeric = toTokenCount(value);
    if (numeric !== null) {
        if (numeric > 0 && numeric % 3600 === 0) {
            return `${numeric / 3600}h`;
        }

        if (numeric > 0 && numeric % 60 === 0) {
            return `${numeric / 60}m`;
        }
...
/**
 * Gets distinct non-empty values and display metadata.
 * @param {string[]} values Values
 * @returns {{value: string|null, title: string}}
 */
function getDistinctValueDisplay(values) {
    ...
    return {
        value: distinctValues.length === 1 ? distinctValues[0] : 'mixed',
        title: distinctValues.join('\n'),
    };
}
...
export function accumulateNanoGptBilling(nanogptMetadata) {
    ...
    const distinctTtls = [...new Set(ttlValues)];
    summary.ttl = distinctTtls.length > 1 ? 'mixed' : distinctTtls[0] ?? null;

    const providerDisplay = getDistinctValueDisplay(providers);
    summary.provider = providerDisplay.value;
    summary.providerTitle = providerDisplay.title;

    const modelDisplay = getDistinctValueDisplay(models);
    summary.model = modelDisplay.value;
    summary.modelTitle = modelDisplay.title;
